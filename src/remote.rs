...
pub(crate) fn run_remote(remote: RemoteLaunch) -> io::Result<()> {
    ...
    let remote_ups = prepare_remote_ups(&remote.target)?;
...
pub(crate) fn run_remote_client_bridge() -> io::Result<()> {
    ...
    let stream = UnixStream::connect(&socket_path).map_err(|err| {
        io::Error::new(
            err.kind(),
            format!(
                "failed to connect to remote Ups client socket {}: {err}",
                socket_path.display()
            ),
        )
    })?;
...
impl RemoteUps {
    fn for_platform(platform: RemotePlatform) -> Self {
        let install_suffix = ".local/bin/ups".to_string();
...
fn prepare_remote_ups(target: &str) -> io::Result<RemoteUps> {
    let platform = detect_remote_platform(target)?;
    let remote_ups = RemoteUps::for_platform(platform);
    let override_binary = remote_binary_override_path()?;

    if override_binary.is_none() && remote_binary_matches(target, &remote_ups)? {
        return Ok(remote_ups);
    }

    confirm_remote_install(
        target,
        &remote_ups,
        &install_source_description(&remote_ups.platform, override_binary.as_deref()),
    )?;
    let source = resolve_install_source(&remote_ups.platform, override_binary)?;
    let install_result = install_remote_ups(target, &remote_ups, &source.path);
    source.cleanup();
    install_result?;

    if !remote_binary_matches(target, &remote_ups)? {
        return Err(io::Error::other(format!(
            "installed remote ups at {}, but it did not report version {CURRENT_VERSION}",
            remote_ups.shell_path
        )));
    }
...
fn remote_binary_matches(target: &str, remote_ups: &RemoteUps) -> io::Result<bool> {
    let command = format!("test -x {0} && {0} --version", remote_ups.shell_path);
    ...
    Ok(version.trim() == format!("ups {CURRENT_VERSION}"))
}
...
fn install_source_description(platform: &RemotePlatform, override_binary: Option<&Path>) -> String {
    ...
    if *platform == RemotePlatform::local() {
        "the current local ups binary".to_string()
    } else {
        format!(
...
fn warn_if_remote_bin_not_on_path(target: &str) -> io::Result<()> {
    ...
    if !output.status.success() {
        eprintln!(
            "ups: installed remote binary to ~/.local/bin/ups, but ~/.local/bin is not in the remote PATH"
        );
    }
    Ok(())
}
...
fn download_release_asset(platform: &RemotePlatform) -> io::Result<InstallSource> {
    ...
    if manifest.version.trim_start_matches('v') != CURRENT_VERSION {
        return Err(io::Error::other(format!(
            "remote host is {}, but this local ups is {CURRENT_VERSION} and the latest release manifest is {}; build ups for the remote platform or install it there manually",
            platform.asset_key(),
            manifest.version
        )));
    }
    ...
    let url = manifest.assets.get(&asset_key).ok_or_else(|| {
        io::Error::other(format!(
            "no {asset_key} binary in the release manifest for ups {CURRENT_VERSION}"
        ))
    })?;

    let dir = private_download_dir(&asset_key)?;
    let path = dir.join("ups.tmp");
...
fn private_download_dir(asset_key: &str) -> io::Result<PathBuf> {
    ...
    Err(io::Error::new(
        ...
        "failed to create private ups remote download directory",
    ))
}

fn confirm_remote_install(
    target: &str,
    remote_ups: &RemoteUps,
    source_description: &str,
) -> io::Result<()> {
    if !io::stdin().is_terminal() {
        return Err(io::Error::other(format!(
            "matching remote ups {CURRENT_VERSION} is not installed at {}; run from an interactive terminal to approve installation",
            remote_ups.shell_path
        )));
    }

    eprintln!(
        "matching ups {CURRENT_VERSION} is not installed on {target} for {}.",
        remote_ups.platform.asset_key()
    );
    eprint!(
        "Install {} to {}? [Y/n] ",
        source_description, remote_ups.shell_path
    );
    io::stderr().flush()?;
    ...
    if answer == "n" || answer == "no" {
        return Err(io::Error::new(
            ...
            "remote ups installation cancelled",
        ));
    }
...
mv "$tmp" "$dest"
"#,
        install_suffix = remote_ups.install_suffix
    );
...
fn remote_bridge_command(remote_ups: &RemoteUps, session_name: &str) -> String {
    let mut command = format!("exec {}", remote_ups.shell_path);
...
fn reattach_command(program: &str, target: &str, session_name: &str) -> String {
    let program = if program.is_empty() { "ups" } else { program };
                ...
                match listener.accept() {
                    Ok((stream, _addr)) => {
                        if let Err(err) = stream.set_nonblocking(false) {
                            eprintln!(
                                "ups: remote bridge failed to prepare client socket: {err}"
                            );
                            continue;
                        }
                        if let Err(err) =
                            bridge_connection(stream, &target, &remote_ups, &session_name)
                        {
                            eprintln!("ups: remote bridge failed: {err}");
                        }
                    }
                    ...
                    Err(err) => {
                        eprintln!("ups: remote bridge listener failed: {err}");
                        break;
                    }
                }
            }
...
fn bridge_connection(
    ...
    command
        .arg("-T")
        .arg(target)
        .arg(remote_bridge_command(remote_ups, session_name));
    command
        .stdin(Stdio::piped())
        .stdout(Stdio::piped())
        .stderr(Stdio::inherit());
...
fn local_forward_socket_path(target: &str, session_name: &str) -> PathBuf {
    ...
    let readable = tmpdir.join(format!(
        "ups-remote-{pid}-{target_clean}-{session_clean}.sock"
    ));
    ...
    let short_name = format!("ups-r-{pid}-{target_prefix}-{hash}.sock");
...
mod tests {
    ...
    fn bridge_socket_is_user_only() {
        ...
        let socket = std::env::temp_dir().join(format!(
            "ups-bridge-permissions-test-{}.sock",
            std::process::id()
        ));
        let remote_ups = RemoteUps::for_platform(RemotePlatform {
            os: "linux",
            arch: "x86_64",
        });
    ...
    fn extract_remote_args_rejects_option_like_target() {
        let args = vec!["ups".into(), "--remote".into(), "-oProxyCommand=x".into()];
        let err = extract_remote_args(&args).unwrap_err();
    ...
    fn reattach_command_includes_remote_and_session() {
        assert_eq!(
            reattach_command("target/release/ups", "user@host", "work"),
            "target/release/ups --remote user@host --session work"
        );
        assert_eq!(
            reattach_command("ups", "host name", crate::session::DEFAULT_SESSION_NAME),
            "ups --remote 'host name'"
        );
    }
    ...
    fn remote_bridge_command_uses_installed_binary() {
        ...
        assert_eq!(
            remote_bridge_command(&remote_ups, crate::session::DEFAULT_SESSION_NAME),
            "exec \"$HOME/.local/bin/ups\" remote-client-bridge"
        );
    }
    ...
    fn install_source_description_uses_override_binary() {
        ...
        assert_eq!(
            install_source_description(&platform, Some(Path::new("/tmp/ups-aarch64"))),
            "HERDR_REMOTE_BINARY (/tmp/ups-aarch64)"
        );
    }
    ...
    fn resolve_install_source_uses_override_binary_without_temporary_cleanup() {
        ...
        let source = resolve_install_source(&platform, Some(PathBuf::from("/tmp/ups-aarch64")))
            .expect("override source");
        assert_eq!(source.path, PathBuf::from("/tmp/ups-aarch64"));
    ...
    fn local_forward_socket_path_uses_readable_name_when_it_fits() {
        ...
        let filename = path
            ...
            .to_string();
        assert!(
            filename.starts_with("ups-remote-"),
    ...
    fn install_source_cleanup_removes_temporary_directory() {
        let dir = std::env::temp_dir().join(format!(
            "ups-install-source-cleanup-test-{}",
            std::process::id()
        ));
        let _ = fs::remove_dir_all(&dir);
